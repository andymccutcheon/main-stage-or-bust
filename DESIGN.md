...
- Hype Press (optional BYOK LLM, `app/press.js`): OpenAI-compatible endpoint, flavor-only JSON {headline, blurb}, 9s timeout, silent fallback to house zine, key kept in memory only, sends just band/venue/result words. The engine never reads AI output — rules stay deterministic and offline-first. Tradeoff accepted: needs network + key; default off.
