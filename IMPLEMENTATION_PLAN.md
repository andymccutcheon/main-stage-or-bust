...
3. `app/sim.js`: heuristic policy (pick higher-fame affordable offer; stage two highest; hype spend if short by <=3; work priority merch-if-stock>0>flyer>job; buy roadie wk>=4 if cash>=40, tech wk>=6 if cash>=45, manager wk>=5 if fame>=15) + 10k-season stats + gate checks (G2). Tune constants until gates pass; record results in BALANCE.md.
## Phase 2 — Web UI (`app/index.html`, `styles.css`, `ui.js`)
Screens: Setup -> Week (offers -> roll -> stage assign -> hype -> work dice -> resolution log) -> Shop -> Season end. Versus: two panels + shared offers. Coop: single sheet, 6-dice tray, "P1 rolled / P2 rolled" labels. Log + tour diary. No framework.
